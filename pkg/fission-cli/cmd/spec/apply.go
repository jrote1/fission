...
func applyArchives(fclient *client.Client, specDir string, fr *FissionResources) error {
	...
	for name, ar := range archiveFiles {
		...
		if url, ok := availableArchives[ar.Checksum.Sum]; ok {
			...
			uploadedAr := _package.UploadArchive(ctx, fclient, ar.URL)
...
func localArchiveFromSpec(specDir string, aus *ArchiveUploadSpec) (*fv1.Archive, error) {
	...
	if _package.FileSize(archiveFileName) < types.ArchiveLiteralSizeLimit {
		contents := _package.GetContents(archiveFileName)
		return &fv1.Archive{
		...
		// checksum
		csum, err := _package.FileChecksum(archiveFileName)
		if err != nil {
